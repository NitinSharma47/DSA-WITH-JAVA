...
public class Shortest_Job_First{
    public static double sjf(int job[]) {

        Arrays.sort(job);                                   // *Sort the jobs , according to their shortest job roles
        int currentWaitingTime = 0;                         // Time lag gya , ab tak (i)
        int totalWaitingTime = 0;                           // total waiting time jo lag gya,  to finish all jobs ....

        for (int i = 0; i < job.length; i++) {
            totalWaitingTime += currentWaitingTime;         // Adds all previous waiting times from every job
            currentWaitingTime += job[i];                   // waiting time till current job
        }

        return (double) totalWaitingTime / job.length;      // calculate average and return in double( for points)
    }
    public static void main(String[] args) {
        int job[] = {3, 1, 4, 2, 5} ;
        System.out.println(sjf(job));
    }
}